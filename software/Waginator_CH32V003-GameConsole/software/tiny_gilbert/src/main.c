...
for (x = 0; x < 128; x++){ 
JOY_OLED_send(SPEED_BLITZ(32,4,x,y,0,start));
}
JOY_OLED_end();
}}
...
uint8_t while1=1;
#define PrecessQuit nn++;if (nn>=128) {while1=0;break;}
JOY_OLED_data_start(0);
for (x=0;x<LIVE-1;x++){
for(t=0;t<4;t++){JOY_OLED_send((sprite26[t]));}
for(t=0;t<4;t++){JOY_OLED_send((sprite27[t]));}
}
for (x=0;x<11+(3-(LIVE-1));x++){
for(t=0;t<8;t++){JOY_OLED_send(0x00);}}
if (((KeyinLevel[levelType])==keyS)&&(timer<=30)) {
for (x=0;x<4;x++){
JOY_OLED_send((sprite12[x]));}}else{
for (x=0;x<4;x++){
for(t=0;t<4;t++){JOY_OLED_send(0x00);}
}}  
for (m = 1; m < 8; m++)
{
JOY_OLED_data_start(m);
n=0; 
nn=0;  
while1=1;
Start=4-step4; 
#define decalIN for(decal=0;decal<DSprite->x4decalage;decal++){JOY_OLED_send(0X00);PrecessQuit}// for main sprite scr pix2pix
#define decalOUT for(decal=0;decal<4-DSprite->x4decalage;decal++){JOY_OLED_send(0X00);PrecessQuit}// for main sprite scr pix2pix
while(while1){
if ((Map[m][n]==7)&&(while1!=0)) {
for(t=Start;t<4;t++){JOY_OLED_send((sprite7[t]));PrecessQuit}Start=0;
// main sprite 
...
if (LorR==1){
if (MainAnim==0) {
for(t=0;t<4;t++){ JOY_OLED_send((sprite20[t])*VSlideOut);PrecessQuit}Start=0;
}
if (MainAnim==1) { 
for(t=0;t<4;t++){ JOY_OLED_send((sprite22[t])*VSlideOut);PrecessQuit}Start=0;
}
if (MainAnim==2) {
for(t=0;t<4;t++){ JOY_OLED_send((sprite24[t])*VSlideOut);PrecessQuit}Start=0; 
} 
}else{  
if (MainAnim==0) {
for(t=0;t<4;t++){ JOY_OLED_send((sprite26[t])*VSlideOut);PrecessQuit}Start=0;   
}
if (MainAnim==1) {
for(t=0;t<4;t++){ JOY_OLED_send((sprite28[t])*VSlideOut);PrecessQuit}Start=0;   
}
if (MainAnim==2) {
for(t=0;t<4;t++){JOY_OLED_send((sprite30[t])*VSlideOut);PrecessQuit}Start=0;  
}}
...
if (LorR==1){
if (MainAnim==0) {
for(t=0;t<4;t++){JOY_OLED_send((sprite21[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;}
}
if (MainAnim==1) {
for(t=0;t<4;t++){ JOY_OLED_send((sprite23[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;}
}
if (MainAnim==2) {
for(t=0;t<4;t++){JOY_OLED_send((sprite25[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;}
} }else{  
if (MainAnim==0) {
for(t=0;t<4;t++){JOY_OLED_send((sprite27[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;}
}
if (MainAnim==1) {
for(t=0;t<4;t++){JOY_OLED_send((sprite29[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;} 
}
if (MainAnim==2) {
for(t=0;t<4;t++){JOY_OLED_send((sprite31[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;}
}}
//fin main sprite
}else if ((Map[m][n]==1)&&(while1!=0)) {
for(t=Start;t<4;t++){JOY_OLED_send((sprite1[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==2)&&(while1!=0)) {
for(t=Start;t<4;t++){JOY_OLED_send((sprite2[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==3)&&(while1!=0)) {
for(t=Start;t<4;t++){JOY_OLED_send((sprite3[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==4)&&(while1!=0)) {
for(t=Start;t<4;t++){JOY_OLED_send((sprite4[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==8)&&(while1!=0)) {
for(t=Start;t<4;t++){JOY_OLED_send((sprite8[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==15)&&(while1!=0)) {
for(t=Start;t<4;t++){JOY_OLED_send((sprite15[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==16)&&(while1!=0)) {
for(t=Start;t<4;t++){JOY_OLED_send((sprite16[t]));PrecessQuit}Start=0;
}
else if ((Map[m][n]==11)&&(while1!=0)) {
if (timer>30){
for(t=Start;t<4;t++){
JOY_OLED_send((sprite11[t]));PrecessQuit}Start=0;
}else{
for(t=Start;t<4;t++){
JOY_OLED_send((sprite12[t]));PrecessQuit}Start=0;
}}else if ((Map[m][n]==13)&&(while1!=0)) {
for(t=Start;t<4;t++){JOY_OLED_send((sprite13[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==14)&&(while1!=0)) {
for(t=Start;t<4;t++){JOY_OLED_send((sprite14[t]));PrecessQuit}Start=0;
}
else{if (while1!=0){
for(t=Start;t<4;t++){
JOY_OLED_send(0x00);PrecessQuit}Start=0;
}}
if (while1!=0) {n++; }
}
JOY_OLED_end();
...
while(1) {
  ...
  JOY_DLY_ms(400);
  while(1){
      if (JOY_act_pressed()) {sound(1);break;}
  }
  RESTARTLEVEL:
  ResetVarNextLevel();
  NEXTLEVEL:
  ...
  if ((JOY_act_pressed())&&(Jump==0)&&(jumpcancel==0)&&(CollisionCheck(&MainSprite)==0)){
      if (MainSprite.y8decalage==0) {Jump=3;}
  }
  if (JOY_act_released()) {jumpcancel=0;}
