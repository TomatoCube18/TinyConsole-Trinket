...
#define PIN_ACT2    PC0   // pin connected to fire button 2
...
#define PIN_UP     PC3   // pin conected to direction buttons (UP)
#define PIN_DOWN   PC6   // pin conected to direction buttons (DOWN)
#define PIN_LEFT   PC4   // pin conected to direction buttons (LEFT)
#define PIN_RIGHT  PC7   // pin conected to direction buttons (RIGHT)
...
#define JOY_act_released()        (PIN_read(PIN_ACT) && PIN_read(PIN_ACT2))
#define JOY_act_pressed()         (!JOY_act_released())


#define JOY_pad_released()        (PIN_read(PIN_UP) && PIN_read(PIN_DOWN) && PIN_read(PIN_LEFT) && PIN_read(PIN_RIGHT))
#define JOY_pad_pressed()         (!JOY_pad_released())

#define JOY_all_released()        (JOY_act_released() && JOY_pad_released())

static inline uint8_t JOY_up_pressed(void) {
 return(!PIN_read(PIN_UP));
}

static inline uint8_t JOY_down_pressed(void) {
 return(!PIN_read(PIN_DOWN));
}
